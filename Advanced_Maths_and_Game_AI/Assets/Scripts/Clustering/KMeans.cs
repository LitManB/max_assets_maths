...
public class KMeans : MonoBehaviour
{
    ...
    public bool CheckOneOfEach()  //--------------------------------------------- checks if one of each ----------------------------------------------------//
    {

        foreach (var cluster in clusteredPoints)
        {
            bool testTube = false, enviroment = false, rats = false, bioAssets = false;

            foreach (var point in cluster.Value)
            {

                if (point.tag == "testTube")
                {
                    testTube = true;
                }
                else
                if(point.tag == "Enviroment")
                {
                    enviroment = true;
                }
                else
                ...
                if (point.tag == "bioAssets")
                {
                    bioAssets = true;
                }
            }

            if (!testTube || !enviroment || !rats || !bioAssets)
                return false;
