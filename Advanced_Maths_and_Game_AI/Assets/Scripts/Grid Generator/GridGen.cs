                ...
                if (!grid[x, y].wall) // --- checks if not a wall 
                {
                    if (pathFinder.FindPath(5, 4, x, y) != null) // ----- checks if a path can be made between the player and grid section ------//
                    {
                        ...
                        if (Random.Range(0, 100) <= 5)  // ----- each grid section has a 5% chance of spawing an object 
                        {
                            ...
                            if (randomNumber == 0)
                            {
                                tempObject = Instantiate(testTube1, new Vector3(x * gridScale, 1, y * gridScale), Quaternion.identity, objectParent.transform); // spawn testTube 
                            }
                            else
                            if (randomNumber == 1)
                            {
                                tempObject = Instantiate(testTube2, new Vector3(x * gridScale, 1, y * gridScale), Quaternion.identity, objectParent.transform); // spawn testTube 
                            }
                            else
                            if (randomNumber == 2)
                            {
                            ...
                            if (randomNumber == 4)
                            {
                                tempObject = Instantiate(bioAssets1, new Vector3(x * gridScale, 1, y * gridScale), Quaternion.Euler(-90, 0, 0), objectParent.transform);
                            }
                            else
                            if (randomNumber == 5)
                            {
                                tempObject = Instantiate(bioAssets2, new Vector3(x * gridScale, 1, y * gridScale), Quaternion.identity, objectParent.transform);
                            }
